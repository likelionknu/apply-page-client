...
export default function ProjectCard({
  ...
  return (
    <>
      {/* 웹 */}
      <article className="relative hidden h-70 w-[389.93px] overflow-hidden rounded-[30px] border border-transparent bg-[linear-gradient(180deg,rgba(0,0,0,0.65)_0%,rgba(23,23,23,0.85)_100%),linear-gradient(#000,#000),linear-gradient(180deg,#ffffff_0%,#76cbf6_100%)] [background-clip:padding-box,padding-box,border-box] bg-origin-border [box-shadow:inset_0_0_0_1px_rgba(255,255,255,0.06),0_10px_32px_rgba(0,0,0,0.55),0_0_28px_rgba(72,185,255,0.14)] min-[701px]:block">
        <div className={innerClass}>
          {item.layout === "center" ? (
            <>
              <h2 className="m-0 mt-0.5 text-center text-[30px] leading-[1.1] font-extrabold tracking-[-0.02em]">
                {displayTitle}
              </h2>

              <div className="mt-2.5 mb-2 flex h-27.5 w-full shrink-0 items-center justify-center">
                <img
                  src={imgSrc}
                  alt={`${item.title} 미리보기`}
              ...
              <p className="m-0 mt-1.5 mb-2 text-center text-[13px] font-extrabold tracking-[-0.015em] text-[rgba(255,255,255,0.92)]">
                {item.subtitle}
              </p>
            </>
          ) : (
              ...
              <div className={rowClass}>
                ...
                >
                  <img
                    src={imgSrc}
                    alt={`${item.title} 미리보기`}
                    className="ilter-[drop-shadow(0_10px_16px_rgba(0,0,0,0.55))] max-h-full max-w-full object-contain opacity-[0.95]"
                  />
                </div>
              </div>
      ...
      {/* 모바일 */}
      <article className="bg-origin-border[box-shadow:inset_0_0_0_1px_rgba(255,255,255,0.08),0_10px_28px_rgba(0,0,0,0.56),0_0_34px_rgba(72,185,255,0.22)] relative h-42.75 w-63.25 overflow-hidden rounded-[34px] border border-transparent bg-[linear-gradient(180deg,rgba(0,0,0,0.60)_0%,rgba(8,14,20,0.94)_100%),linear-gradient(#000,#000),linear-gradient(180deg,rgba(230,246,255,0.95)_0%,rgba(120,203,255,0.92)_100%)] [background-clip:padding-box,padding-box,border-box] min-[701px]:hidden">
        <div className="relative h-42.75 px-3.75 pt-3.5 pb-3">
          <h2 className="m-0 mt-2 ml-2 bg-linear-to-b from-cyan-300 via-sky-300 to-indigo-300 bg-clip-text text-[15px] leading-[1.05] font-semibold tracking-[-0.03em] text-transparent">
            {mobileTitle}
          </h2>
